...
import WannaChat from "./WannaChat/WannaChat";
...
const Chat = () => {
    ...
    const [showWannaChat, setShowWannaChat] = useState(false);
    ...
    const handleKeyDown = (e: React.KeyboardEvent) => {
        if (e.key === 'Enter' && !e.shiftKey) {
            e.preventDefault(); // ← Add this to prevent default Enter behavior
            
            if (editableRef.current) {
                const content = editableRef.current.innerText;
                setInitMessage(content);
            }
            setShowWannaChat(true);
        }
    };
    ...
    return (
        <div className={styles.chat}>
            {showWannaChat ? (
                <WannaChat initMessage={initMessage} />
            ): (
